...
- Move the guessing game to a separate HTML file, link it to/from your `index.html` file, list the questions on the page, and then have the answers/responses fill in on the page after the user provides the answers (note that they will not appear until after the user has completed the entire guessing game).
